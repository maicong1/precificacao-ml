...
# Custom CSS para layout mobile e clean
st.markdown(
    '''
    <style>
    html, body, [class*="css"]  {
    ...
    .main .block-container {
        padding-top: 0rem; /* Remove espaço antes do título */
    }
    .stApp {
        height: 100%;
        display: flex;
    ...
    .stMarkdown h1 {
        text-align: center;
        margin-bottom: 0.3em;
        font-size: 1.5em; /* 50% menor */
    }
    /* Remove menu hamburger e botões extra */
...
if "historico" not in st.session_state:
    st.session_state.historico = pd.DataFrame(columns=[
        "Produto", "Custo Unitário (R$)", "Frete (R$)", "Outros Custos (R$)",
        "Preço Premium (R$)", "Lucro Bruto Premium (R$)", "Margem Premium (%)", "Markup Real Premium (%)",
    ...
    if st.button("Recalcular"):
        ...
        st.session_state.historico = pd.concat([
            ...
            pd.DataFrame([{
                ...
                "Custo Unitário (R$)": st.session_state.custo_unitario,
                "Frete (R$)": st.session_state.frete,
                "Outros Custos (R$)": st.session_state.outros_custos,
